...
func (h *Inbound) fallbackConnection(ctx context.Context, conn net.Conn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) {
		...
		if tlsConn, loaded := common.Cast[tls.Conn](conn); loaded {
			...
			if connectionState.NegotiatedProtocol != "" {
				if fallbackAddr, loaded = h.fallbackAddrTLSNextProto[connectionState.NegotiatedProtocol]; !loaded {
					h.logger.WarnContext(ctx, "fallback disabled for ALPN: ", connectionState.NegotiatedProtocol)
					return
				}
			}
		}
	...
	if !fallbackAddr.IsValid() {
		if !h.fallbackAddr.IsValid() {
			h.logger.WarnContext(ctx, "fallback disabled by default")
			return
		}
...
func (t *inboundTransportHandler) FallbackConnection(ctx context.Context, conn net.Conn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	(*Inbound)(t).fallbackConnection(ctx, conn, adapter.InboundContext{
		Source:      source,
